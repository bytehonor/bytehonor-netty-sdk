 ...
 */
@Deprecated
public class NettyServerSender {

    private static final Logger LOG = LoggerFactory.getLogger(NettyServerSender.class);
...
//        doSendBytes(channel, bytes);
    ...
    public static void send(String stamp, NettyPayload payload) {
        ...
        byte[] bytes = NettyDataUtils.build(payload.toString());
        doSendBytes(channel, bytes);
    }
